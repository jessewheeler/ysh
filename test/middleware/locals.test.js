...
describe('injectLocals middleware', () => {
  test('populates res.locals.site from site_settings table', () => {
  ...
  test('sets isAdmin true when session.adminId is set', () => {
    const res = buildRes();
    injectLocals(buildReq({ adminId: 1 }), res, jest.fn());
    expect(res.locals.isAdmin).toBe(true);
  });

  test('sets isAdmin false when session.adminId is missing', () => {
    const res = buildRes();
    injectLocals(buildReq(), res, jest.fn());
    expect(res.locals.isAdmin).toBe(false);
  });

  test('sets adminRole from session', () => {
    const res = buildRes();
    injectLocals(buildReq({ adminId: 1, adminRole: 'super_admin' }), res, jest.fn());
    expect(res.locals.adminRole).toBe('super_admin');
  });

  test('sets adminRole to null when not logged in', () => {
    const res = buildRes();
    injectLocals(buildReq(), res, jest.fn());
    expect(res.locals.adminRole).toBeNull();
  });

  test('sets adminEmail from session', () => {
    const res = buildRes();
    injectLocals(buildReq({ adminId: 1, adminEmail: 'admin@test.com' }), res, jest.fn());
    expect(res.locals.adminEmail).toBe('admin@test.com');
  });

  test('sets adminEmail to null when not logged in', () => {
    const res = buildRes();
    injectLocals(buildReq(), res, jest.fn());
    expect(res.locals.adminEmail).toBeNull();
  });

  test('sets currentPath from req.path', () => {
    const req = buildReq();
    req.path = '/membership';
    const res = buildRes();
    injectLocals(req, res, jest.fn());
    expect(res.locals.currentPath).toBe('/membership');
  });

  test('copies flash_success to locals and deletes from session', () => {
  ...
  test('copies flash_error to locals and deletes from session', () => {
  ...
  test('handles missing flash messages gracefully', () => {
    const res = buildRes();
    injectLocals(buildReq(), res, jest.fn());
  ...
  test('always calls next()', () => {
    const next = jest.fn();
    injectLocals(buildReq(), buildRes(), next);
